...
function Bot.initialize(agent)
	...
	local brain=dofile(agent:getScriptClassPath() .. "\\MLP.lua");
	
	brain.init(0.1);
	brain.addLayer(13); --input layer
	brain.addLayer(13); --hidden layer
	brain.addLayer(5); --output layer	
	
	Bot[agentId]={};
	Bot[agentId].brain=brain;
end
...
function Bot.train()
	local dataFile=agent:getScriptClassPath() .. "\\data.lua";
	if fileExists(dataFile)==false then
		alert("File Not Found", dataFile .. " does not exists");
		return;
	end
	local records=dofile(dataFile);
	...
	alert("Training completed", "Training Completed with errors " .. err);
end
