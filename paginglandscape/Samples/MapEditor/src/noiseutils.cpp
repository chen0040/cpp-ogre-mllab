...
namespace noise
{
  ...
  namespace utils
  {
    ...
    inline noise::uint8 BlendChannel (const uint8 channel0,
									  const uint8 channel1, 
									  float alpha)
    {
	  const float invScale = 1 / 255.0f;
      const float c0 = (float)channel0 * invScale;
      const float c1 = (float)channel1 * invScale;
...
void WriterBMP::WriteDestFile ()
{
  ...
  for (int y = 0; y < height; y++) {
	...
	os.write ((char*)pLineBuffer, (std::streamsize)bufferSize);
    if (os.fail () || os.bad ()) {
      os.clear ();
      os.close ();
      os.clear ();
