...
private:
	...
	float *m_UKnots;
	float *m_VKnots;
	float *m_UBasis, *m_dUBasis;
	float *m_VBasis, *m_dVBasis;
	Point4D *m_UTemp, *m_dUTemp;
	int *m_TessUKnotSpan;
	int *m_TessVKnotSpan;
...
public:
	CDRGNURBSSurface( );
	virtual ~CDRGNURBSSurface( );

	bool Init( int uDegree, int vDegree, int uControlPoints, int vControlPoints, Point4D* pControlPoints, float* pUKnots, float* pVKnots, int iDefaultUTessellations = 10, int iDefaultVTessellations = 10 );
				
	void UpdateControlPoints( Point4D *pControlPoints );

	splinePoint getData( int index ) const;
