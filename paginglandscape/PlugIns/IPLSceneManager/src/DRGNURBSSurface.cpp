...
{
	if ( ( iUTessellations != m_iUTessellations ) || ( iVTessellations != m_iVTessellations ) )
	{
...
void CDRGNURBSSurface::ComputeBasisCoefficients( void )
{
	...
	for ( i = 0; i < m_iUBasisSpans; i++ )
	{
		for ( j = 0; j < m_iUOrder; j++ )
		{
			...
			{
				m_UBasisCoefficients[ ( i * m_iUOrder + j ) * m_iUOrder + k ] = ComputeCoefficient( m_UKnots, i + m_iUDegree, i + j, m_iUDegree, k );
			}
		}
	}
	...
	for ( i = 0; i < m_iVBasisSpans; i++ )
	{
		for ( j = 0; j < m_iVOrder; j++ )
		{
			...
			{
				m_VBasisCoefficients[ ( i * m_iVOrder + j ) * m_iVOrder + k ] = ComputeCoefficient( m_VKnots, i + m_iVDegree, i + j, m_iVDegree, k );
			}
		}
	}
...
void CDRGNURBSSurface::EvaluateBasisFunctions( void )
{
	...
	//
	// First evaluate the U basis functions and derivatives at uniformly spaced u values
	//
	idx = 0;
	...
	for ( i = 0; i <= m_iUTessellations; i++ )
	{
		...
		for ( j = 0; j < m_iUOrder; j++ )
		{
			...
			{
				m_UBasis[ ( i * m_iUOrder + j ) * SIMD_SIZE ] = m_UBasis[ ( i * m_iUOrder + j ) * SIMD_SIZE ] * u + m_UBasisCoefficients[ ( idx * m_iUOrder + j ) * m_iUOrder + k ];
				if ( k > 0 )
				{
					m_dUBasis[ ( i * m_iUOrder + j ) * SIMD_SIZE ] = m_dUBasis[ ( i * m_iUOrder + j ) * SIMD_SIZE ] * u + m_UBasisCoefficients[ ( idx * m_iUOrder + j ) * m_iUOrder + k ] * k;
				}
			}
			//
	...
	for ( i = 0; i <= m_iVTessellations; i++ )
	{
		...
		for ( j = 0; j < m_iVOrder; j++ )
		{
			...
			{
				m_VBasis[ ( i * m_iVOrder + j ) * SIMD_SIZE ] = m_VBasis[ ( i * m_iVOrder + j ) * SIMD_SIZE ] * v + m_VBasisCoefficients[ ( idx * m_iVOrder + j ) * m_iVOrder + k ];
				if ( k > 0 )
				{
					m_dVBasis[ ( i * m_iVOrder + j ) * SIMD_SIZE ] = m_dVBasis[ ( i * m_iVOrder + j ) * SIMD_SIZE ] * v + m_VBasisCoefficients[ ( idx * m_iVOrder + j ) * m_iVOrder + k ] * k;
				}
			}
		}
		v += vinc;
...
//
// -----------------------------------------------------------------------
void CDRGNURBSSurface::TessellateSurface( void )
{
	int u, v;
	int k, l;
	int uKnot, vKnot;
...
// -----------------------------------------------------------------------
splinePoint CDRGNURBSSurface::getData( int index ) const
{
...
//
// -----------------------------------------------------------------------
int CDRGNURBSSurface::GetTriangleCount( void ) const
{
