...
namespace Ogre
{
				...
				while (pIt.hasMoreElements ())
				{
					...
					while (tuIt.hasMoreElements ())
					{
							...
							StringUtil::startsWith (texName, opt->image_filename))
						{
							...
							//if (!mTextures[channel]->isLoaded())
							//	mTextures[channel]->load();
						...
						else if (StringUtil::startsWith (texName, filename, false))
						{
								...
								&& texType ==  "Light" )
							{
								...
								PLSM2_ASSERT (!mLightTexture.isNull() && 
									String(texName + " is missing").c_str());
								mLightBuffer = mLightTexture->getBuffer();
								loadTexture (texName, mLightImage);

								PLSM2_ASSERT (mLightBuffer->getWidth () ==  mLightImage.getWidth () && 
									...
									resourceExists (opt->groupName, shadowTexName))
								{
									...
									PLSM2_ASSERT (mShadow.getWidth () ==  mLightImage.getWidth () && 
...
#ifdef PLSM2_EIHORT
                numTextureUnits = std::max (numTextureUnits, static_cast<size_t>(p->getNumTextureUnitStates()));
#else			
		...
		if (mMaterial.isNull())
		{
			...
			if (opt->materialPerPage)
			{
				...
				mMaterial = MaterialManager::getSingleton().getByName(
					mMaterialBaseName + nameSep + commonName);

				if (mMaterial.isNull())
				{
					mMaterial = MaterialManager::getSingleton().load(mMaterialBaseName + nameSep + commonName, ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME);

					if (mMaterial.isNull())
					{
						LogManager::getSingleton().logMessage(LML_CRITICAL,
							String("PLSM2 : Cannot find material named ") + mMaterialBaseName + nameSep + commonName);
						return;
					}
				}
				{
					...
					while (tIt.hasMoreElements ())
					{
						...
						while (pIt.hasMoreElements ())
						{
							...
							{
								TextureUnitState * const tu = tuIt.getNext ();
								if (tu->getContentType() != TextureUnitState::CONTENT_SHADOW)
								{
									const String texType (tu->getName());
									if (!texType.empty() && std::string::npos == texType.find("."))
									{
										bool bFound = false;
						...
						while (pIt.hasMoreElements ())
						{
							...
							while (tuIt.hasMoreElements ())
							{
								...
								if (std::string::npos == texType.find("."))
								{
									...
									else if (StringUtil::startsWith (texType, "normal", true))
									{
										texName = beginName + texType + endName;
										channel++;
										deformable = true;
									...
									else if (StringUtil::startsWith (texType, "normalheightmap", true))
									{
										texName = beginName + texType + endName;
										channel++;
										deformable = true;
									...
									else if (StringUtil::startsWith (texType, "coveragemap", true))
									{
										//texName = beginName + texType + nameSep;
										String outBase, OutPath;
										StringUtil::splitFilename (beginName, outBase, OutPath);
										texName = OutPath + texType + nameSep;									
										texName += StringConverter::toString((coveragechannel * 3)  % opt->NumMatHeightSplat) + endName;
										deformable = true;
										channel++;
										coveragechannel++;
									}						
	...
	void PagingLandScapeTexture::unload()
	{
        if (mIsLoaded)
        {
			...
			if (!mMaterial.isNull())
			{
				...
				PLSM2_ASSERT (!mMaterial.isNull() && "PagingLandScapeTexture::unload");      
				mMaterial->unload();
				mMaterial.setNull();
				if (!mParent->getOptions()->materialPerPage)
	...
	{
		PLSM2_ASSERT (!mMaterial.isNull() && "PagingLandScapeTexture::::_save");  
			...
			for (unsigned int i = 0; i < mNumTexture; i++)
			{
				if (isTextureModified[i])
				{
					...
					if (it != finfo->end())
					{
						...
						PLSM2_ASSERT (mImages[i].getData ());
	...
	{    
		PLSM2_ASSERT (mNumTexture > 0);
		PLSM2_ASSERT (!mMaterial.isNull() && "PagingLandScapeTexture::update()");

		for (unsigned int i = 0; i < mNumTexture; i++)
		{
			if (doTextureNeedUpdate[i])
			{
				PLSM2_ASSERT (!mTextures[i].isNull() && "PagingLandScapeTexture::update()");
				PLSM2_ASSERT (!mBuffers[i].isNull() && "PagingLandScapeTexture::update()");
				PLSM2_ASSERT (mImages[i].getData () != 0 && "PagingLandScapeTexture::update()");
	...
	{	
		...
		PLSM2_ASSERT (mImages[channel].getHeight() == mImages[channel].getWidth());
		PLSM2_ASSERT ((float)mImages[channel].getHeight() / (mParent->getOptions()->PageSize - 1)
			== mParent->getOptions()->TextureStretchFactor && 
        ...
        const Image::Box& textureRect)
    {
        ...
        PLSM2_ASSERT (heightData && "PagingLandScapeTexture::compute()");
        ...
        for (size_t k = textureRect.top; k < textureRect.bottom; ++k)
        {
            ...
            {             
                ...
                PLSM2_ASSERT (i < textureSize && k < textureSize && 
                    "PagingLandScapeTexture::compute()");

                PLSM2_ASSERT (i_terrain < heightfiledsize && k_terrain < heightfiledsize && 
                    "PagingLandScapeTexture::compute()");
	...
	{
		PLSM2_ASSERT (mNumTexture > 0);
		PLSM2_ASSERT (paintForce >= 0.0f && paintForce <= 1.0f && "PagingLandScapeTexture::paint()");
        ...
        PLSM2_ASSERT (((xParam + zParam *  psize) < psize*psize) && "PagingLandScapeTexture::paint()");
			...
			if (z < pSizeScaled)
			{
				...
				for (unsigned int k = 0; k < textureScaleCount; k++)
				{
					...
					if (x < pSizeScaled)
					{
						...
						PLSM2_ASSERT (curr_image_pos < pSizeScaled*pSizeScaled);
    ...
    {
		PLSM2_ASSERT (mNumTexture > 0);
        PLSM2_ASSERT (x < (mParent->getOptions ()->PageSize));
        PLSM2_ASSERT (z < (mParent->getOptions ()->PageSize));
    ...
    {
		PLSM2_ASSERT (mNumTexture > 0);
        PLSM2_ASSERT (x < ((mParent->getOptions ()->PageSize - 1) * mParent->getOptions ()->TextureStretchFactor));
        PLSM2_ASSERT (z < ((mParent->getOptions ()->PageSize - 1) * mParent->getOptions ()->TextureStretchFactor));
	...
	{		
		...
		if (mIsShaderShadowed)
		{
			...
			PLSM2_ASSERT (fabs (SunAngle) < 1.1f);
		...
		{
			PLSM2_ASSERT (mLightImage.getData ());
			computeLightMap ();
	...
	{    
		...
		PLSM2_ASSERT ((mShadow.getBPP ()) / 8 == 3);
		...
		const Real slope)
	{
		...
		{    
			if (mImages[k].getBPP () == 8)
			{
				...
				PLSM2_ASSERT (BaseData && "PagingLandScapeTexture::computePointAlpha()");
				const unsigned int curr_image_pos = imagePos*(static_cast <unsigned int>(mNumChannelperTexture[k]));
				PLSM2_ASSERT (mNumChannelperTexture[k] == (mImages[k].getBPP ()/8));
		...
		const Real slope)
	{
		...
		{    
			...
			{
				uchar * const BaseData = mImages[k].getData ();
				PLSM2_ASSERT (BaseData && "PagingLandScapeTexture::computePointColor()");
				const unsigned int curr_image_pos = imagePos*(static_cast <unsigned int>(mNumChannelperTexture[k]));
				PLSM2_ASSERT (mNumChannelperTexture[k] == (mImages[k].getBPP ()/8));
	...
	{
		PLSM2_ASSERT (mNumTexture > 0);
		PLSM2_ASSERT(mParent->channelModifList);
		...
		for (unsigned int k = 0; k < mNumTexture; k++)
		{
			...
			PLSM2_ASSERT (BaseData && "PagingLandScapeTexture::paint()");
			const unsigned int currImagePos = imagePos * mNumChannelperTexture[k];
			PLSM2_ASSERT (currImagePos < mImages[k].getSize ());
			PLSM2_ASSERT (mNumChannelperTexture[k]*8 == mImages[k].getBPP ());
