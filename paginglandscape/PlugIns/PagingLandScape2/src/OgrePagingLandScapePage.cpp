...
namespace Ogre
{
	...
	{ 
		PLSM2_ASSERT (!mIsLoading);
		PLSM2_ASSERT (!mIsPreLoading);
		PLSM2_ASSERT (!mIsTextureLoading);
		PLSM2_ASSERT (!mIsUnloading);

		PLSM2_ASSERT (!mIsPostUnloading);
		PLSM2_ASSERT (!mIsTextureunloading);
		PLSM2_ASSERT (!mIsLoaded);
		PLSM2_ASSERT (!mIsTextureLoaded);
		PLSM2_ASSERT (!mIsPreLoaded);
		PLSM2_ASSERT (mIsLoadable);
	...
	void PagingLandScapePage::_setNeighbor(const Neighbor& n, PagingLandScapePage* p)
	{
		...
		PLSM2_ASSERT (!thisLoaded || (thisLoaded && !mTiles.empty()));
    ...
    void PagingLandScapePage::unload()
	{
        if (mIsLoaded)
        {
            ...
            PLSM2_ASSERT (mPageNode->getParent () == 0);
    ...
    PagingLandScapeTile *PagingLandScapePage::getTile(const Vector3& pos)
    {
        if (mIsLoaded && mIsLoadable)
        {
            ...
            PLSM2_ASSERT (mTiles[x][z] && mTiles[x][z]-> isLoaded ());
            return mTiles[x][z];
        }
        return 0;
    }
    ...
    PagingLandScapeTile* PagingLandScapePage::getTile(const unsigned int i , const unsigned int j) const
    {
        if (mIsLoaded)
        {
            ...
            PLSM2_ASSERT (i < mParent->getOptions()->NumTiles);
            PLSM2_ASSERT (j < mParent->getOptions()->NumTiles);

            PLSM2_ASSERT (i < mTiles.size());
            PLSM2_ASSERT (j < mTiles[i].size());
