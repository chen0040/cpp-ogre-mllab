...
//    }
                                                                ...
                                                                Ogre::Vector3 * rayresult)
    {
		...
		PLSM2_ASSERT (mPageManager && mData2DManager);
...
// 				raydir.z * mOptions->scale.z);
    ...
    void PagingLandScapeSceneManager::resizeCrater ()
    {
        ...
        for (int Zcurr = Zmin; Zcurr < Zmax; Zcurr++)
        {
            ...
            if (Precalc > 1.0f)
            {
                ...
                {
                    PLSM2_ASSERT (curr_indx < array_size);
				...
				for (int Xcurr = Xmin; Xcurr < Xmax; Xcurr++)
				{
					...
					if (h - 0.005f > 0)
					{
						...
						if (bushForce - 0.005f > 0)
						{
							...
							PLSM2_ASSERT (mPageManager&&  mData2DManager && mTextureManager);
							...
							{ 
								...
								PLSM2_ASSERT (curr_indx < mBrushArrayWidth*mBrushArrayHeight);   
    ...
    bool PagingLandScapeSceneManager::setOption(const String& strKey, const void* pValue)
    {
        ...
        {
            if (mListenerManager && mListenerManager->setOption (strKey, pValue))
				return true;
			else
				return false;
		} 
...
#ifndef _PLSM_OCTREE
		if (OctreeSceneManager::setOption (strKey, pValue) == true)
#else //_PLSM_OCTREE
	...
	bool PagingLandScapeSceneManager::getOption(const String& strKey, void* pDestValue)
    {
		...
		{
			#ifndef _PLSM_OCTREE
					return OctreeSceneManager::getOption (strKey, pDestValue);
			#else //_PLSM_OCTREE
    ...
    bool PagingLandScapeSceneManager::hasOption(const String& strKey) const
    {
		...
		{
			#ifndef _PLSM_OCTREE
					return OctreeSceneManager::hasOption (strKey);
			#else //_PLSM_OCTREE
...
#ifndef _PLSM_OCTREE
		return OctreeSceneManager::getOptionKeys (refKeys);
#else //_PLSM_OCTREE
...
#ifndef _PLSM_OCTREE
		OctreeSceneManager::destroyAllCameras();
#else //_PLSM_OCTREE
