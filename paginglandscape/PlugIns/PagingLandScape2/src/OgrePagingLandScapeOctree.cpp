    ...
    PagingLandScapeOctree::~PagingLandScapeOctree()
    {
        ...
        for (unsigned int i = 0; i < 2; i++)
        {
            for (unsigned int j = 0; j < 2; j++)
            {
                for (unsigned int k = 0; k < 2; k++)
                {
                    PLSM2_ASSERT (mChildren[ i ][ j ][ k ] == 0);
                }
            }
        }
#endif //_DEBUG
    ...
    */
    ...
    { 
        ...
        */
        ...
        PLSM2_ASSERT (octantMax.x >= ncenter.x && octantMax.y >= ncenter.y && octantMax.z >= ncenter.z &&
        ...
        if (mChildren[ x ][ y ][ z ] == 0)
        {
            ...
            PLSM2_ASSERT (max.x >= ncenter.x && max.y >= ncenter.y && max.z >= ncenter.z &&
    ...
    {  
        PLSM2_ASSERT (!mNodes.empty());

		PagingLandscapeOctreeNodeList::iterator it;
