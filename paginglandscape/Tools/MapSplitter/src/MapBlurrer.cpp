...
#define uimin(a,b) (a>b?b:a)
    ...
    void Convolver::blurRgba (Image  * img)
    {
        ...
        int * const ogre_restrict r = new int[wh];
        int * const ogre_restrict g = new int[wh];
        int * const ogre_restrict b = new int[wh];
        int * const ogre_restrict a = new int[wh];

        uint k = 0;
        uchar * const ogre_restrict pix = img->getData ();
        for (int i = 0; i < wh; i++ )
        {
            r[i] = pix[k];
            g[i] = pix[k+1];
        ...
        int * const ogre_restrict r2 = new int[wh];
        int * const ogre_restrict g2 = new int[wh];
        int * const ogre_restrict b2 = new int[wh];
        int * const ogre_restrict a2 = new int[wh];
   ...
   //-----------------------------------------------------------------------
    void MapBlurrer::gaussianBlur(Image *BlurImage, int sigma)
    {
            ...
            for(uint x = 0; x < width; ++x)
            {
                for(uint z = 0; z < Bpp; ++z)
                {
                    ...
                    for(int kx = -kernelRadius; kx <= kernelRadius; ++kx)
                    {
                        ...
                        int pixelColumn = static_cast <int> ((x + kx)*Bpp);
                ...
                for (uint z = 0; z < Bpp; ++z)
                {
                    ...
                    for (int kx = -kernelRadius; kx <= kernelRadius; ++kx)
                    {
                        ...
                        if(pixelRow >= (int)height)
                            pixelRow = static_cast <int> (height - (Bpp - z));
        ...
        if(kernel)
            delete [] kernel;
        kernel = NULL;
        DEBUG_END_SECTION_OUTPUT (String("\n"));
    }
}//namespace Ogre
