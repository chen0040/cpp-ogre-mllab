...
namespace Ogre
{
    ...
    void MapMergeModulater::ModulateGrayAndRgbInRgb(Image *GrayImage, Image *RgbImage, 
                                uint factor)
    {
        uchar * ogre_restrict DataRGB = RgbImage->getData();
        uchar * ogre_restrict DataGray = GrayImage->getData();
    ...
    void MapMergeModulater::ModulateGrayAndRgbaInRgba (Image *GrayImage, Image *RgbaImage, 
                                                    uint factor)
    {
        uchar * ogre_restrict DataRGBA = RgbaImage->getData();
        uchar * ogre_restrict DataGray = GrayImage->getData();
    ...
    Image *MapMergeModulater::MergeGrayAndRgbInRgba(Image *GrayImage, Image *RgbImage)
    {
		...
		//uchar * ogre_restrict DataRGBA = new uchar[ image_width*image_height*4 ];   
		uchar * ogre_restrict DataRGBA = OGRE_ALLOC_T(uchar, image_width*image_height*4, MEMCATEGORY_GENERAL);
        ...
        RgbaImage->loadDynamicImage (DataRGBA, static_cast <size_t> (image_width),
                                     static_cast <size_t> (image_height), 1, PF_R8G8B8A8, true); 
    ...
    {
        uchar * ogre_restrict DataRGB = RgbImage->getData();
        uchar * ogre_restrict DataAlpha = AlphaImage->getData();
        const uint image_width = static_cast <uint> (RgbImage->getWidth ());
        const uint image_height = static_cast <uint> (RgbImage->getHeight ()); 
