...
namespace Ogre
{
    ...
    {}
    //-----------------------------------------------------------------------
    void MapSplitter::computePageSize()
   {
        size_t pageh = mCurrentMap->getHeight() /
                    MapUtil::getSingleton().mPageZ;
        size_t pagew = mCurrentMap->getWidth() /
            MapUtil::getSingleton().mPageX;
    ...
    //-----------------------------------------------------------------------
    void MapSplitter::setTerrainSave (bool option)
    {
        b_SaveAsTerrain = option;
    }
    //-----------------------------------------------------------------------
    void MapSplitter::init (const bool paged, const uint pagesize, const uint bpp, 
                            const String &outDir)
    {
        if (paged)
        {
    ...
    void MapSplitter::saveAndSplitMap (const String &filename, Image *newImage)
    {
        ...
        if (b_SaveBigFile)
            ...
            SplitCurrentMap (filename, MapUtil::getSingleton().mPageSize + 1);
    ...
    void MapSplitter::saveAndSpaceAndSplitMap (const String &filename, Image *newImage)
    {
        ...
        if (b_SaveBigFile)
            ...
            SplitAndSpaceCurrentMap (filename, mTileSpacing, MapUtil::getSingleton().mPageSize + 1);
                                                ...
                                                const int tile_spacing, const int page_size)
    {
        ...
        const uint base_image_bypp = mBpp;
        const uint size = base_image_width * base_image_height;
        uchar * const BaseData = new uchar[size * base_image_bypp];
        if (base_image_bypp >= 2)
        {
            ...
            { 
                ...
                const ushort syn = static_cast <ushort>  (Heightdata[i] * divider);
                #if OGRE_ENDIAN == OGRE_ENDIAN_BIG
                    BaseData[j] = static_cast <uchar> ((syn >> 8) & 0xff);
		            BaseData[j+ 1] = static_cast <uchar>  (syn & 0xff);
                #else
		            BaseData[j] = static_cast <uchar>   (syn & 0xff);
		            BaseData[j+ 1] = static_cast <uchar>   ((syn >> 8) & 0xff);
                #endif
                j += base_image_bypp;            
            }
        }
        ...
        {
            const double divider = (1.0 / MapUtil::getSingleton().getMaxTheoHeight()) * 255;
            for (uint i = 0; i < size; i++ )
            {             
                BaseData[i] = static_cast <uchar> (Heightdata[i] * 256);
            }
        }
                                                ...
                                                const bool saveasRaw, const String &extension)
    {
		...
		DEBUG_OUTPUT (String("Width : ") + StringConverter::toString(base_image_width));
        DEBUG_OUTPUT (String("  Height : ") + StringConverter::toString(base_image_height));
        DEBUG_OUTPUT (String("  Bypp : ") +  StringConverter::toString(base_image_bypp));
        ...
        DEBUG_OUTPUT (String("PureFileName : ") + pure_file_name );
        DEBUG_OUTPUT (String("Extension : ") + ext_name );
        ...
        for (i = 0; i < height_split_count; i++)
        {
            for (j = 0; j < width_split_count; j++)
            {
                ...
                save (const_cast< char * > (SplitFileName.c_str()),
                    ...
                    page_size + tile_spacing,
                    base_image_bypp);
                DEBUG_OUTPUT (String("created : ") + SplitFileName );
            }
         }      
   ...
   //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    const bool MapSplitter::saveRaw(const String &filename, Real* ogre_restrict hdata,
                                    const uint width, const uint height)
    {
        const uint base_image_bypp = mBpp;
        uchar * const ogre_restrict data = new uchar[width * height * base_image_bypp];   
        if (base_image_bypp == 2)
        {
            uint j = 0;
    ...
    //-----------------------------------------------------------------------
    const bool MapSplitter::saveExt(const String &filename,  Real* ogre_restrict hdata,
                                    const uint width, const uint height,
                                    const uint NumBytesPP)
    {
        ...
        uchar * const ogre_restrict data = new uchar[imgData->size];
        ...
        if (b_SaveAsTerrain)
            ext_name = (b_SaveAsRaw)? MapUtil::getSingleton().mExt:MapUtil::getSingleton().mExt;
        else
   ...
   //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    const bool MapSplitter::save(const String &filename, uchar * const data,
                                const uint width, const uint height,
                                const uint NumBytesPP)
     {
        bool Okraw = false;
        ...
        if (b_SaveAsRaw)// && NumBytesPP == 2)//raw 16 bits image.
    ...
    //-----------------------------------------------------------------------
    const bool MapSplitter::saveExt(const String &filename, uchar * const data,
        const uint width, const uint height,
        const uint NumBytesPP)
    {
        // Get codec according to filename extension
        String ext_name;
        if (b_SaveAsTerrain)
            ext_name = (b_SaveAsRaw)? "Raw":MapUtil::getSingleton().mExt;
        else
            ext_name = MapUtil::getSingleton().mTextureExt;
...
//        imgData->size = width*height*NumBytesPP;
        ...
        if (NumBytesPP == 1)
            ...
            fmt = PF_R8G8B8;//PF_R8G8B8;
        else if (NumBytesPP == 4)
            fmt = PF_R8G8B8A8;//PF_R8G8B8A8;
            
        //imgData->depth =  1;
        //imgData->format = fmt;
        
        Image tmpImage;
        tmpImage.loadDynamicImage(data, width, height, fmt);
        tmpImage.save(pure_file_name + ext_name);
...
//        pCodec->codeToFile(image_chunk, pure_file_name + ext_name, codecdata);
                                    ...
                                    uint bpp)
    {
        ...
        const uchar * const ogre_restrict src = BaseData + y*src_shift + bpp*x;
        uchar * const dest = OutData;
        uint destIdx = 0;
        uint srcIdx = 0;
        for (uint j = 0; j < tile_height_copy; j++)
        {
            memcpy (&dest[destIdx], &src[srcIdx], dest_shift_copy);
            destIdx += dest_shift_copy;
            srcIdx += src_shift;
            memset (&dest[destIdx], dest[destIdx - 1], dest_shift_fill);
            destIdx += dest_shift_fill;
        }
        const uint fill_height = tile_height - tile_height_copy;
        if (fill_height > 0)
        {
            srcIdx = destIdx -  dest_shift;
            for (uint k = 0; k < fill_height; k++)
            {
                memcpy (&dest[destIdx], &dest[srcIdx], dest_shift);
                destIdx += tile_width;
            }
        }
    }
                                    ...
                                    const uint bpp)
    {
        ...
        const uchar *  const ogre_restrict src = BaseData + y*src_shift + bpp*x;
        uchar * const ogre_restrict dest = OutData;
        uint destIdx = 0;
        uint srcIdx = 0;
        for (uint j = 0; j < tile_height_copy; j++)
        {
                                                ...
                                                const int page_size)
    {
        ...
        const unsigned int height_split_count = MapUtil::getSingleton().mPageZ - 1;//base_image_height / page_size;
        const unsigned int width_split_count = MapUtil::getSingleton().mPageX - 1;//base_image_width / page_size;
        ...
        if ((base_image_height / (page_size - tile_spacing)) == 1 &&
            (base_image_width / (page_size - tile_spacing)) == 1)
        {
        ...
        for (i = 0; i < height_split_count; i++)
        {
            ...
            {
				//uchar *SplittedData = new uchar [size_split_count * base_image_bypp];
				uchar *SplittedData = OGRE_ALLOC_T(uchar, size_split_count * base_image_bypp, MEMCATEGORY_GENERAL);
...
//                page_size + tile_spacing,
//                base_image_bypp);
            DEBUG_OUTPUT ("created : " + SplitFileName );
        }
        //----last row, last col : reads too far on right and bottom
        // Blit copy from base image to sub-image
...
//            base_image_bypp);
    ...
    void MapSplitter::SplitCurrentMap(const String &filename, const int page_size)
    {
        ...
        DEBUG_OUTPUT (String("Width : ") + StringConverter::toString(base_image_width));
        DEBUG_OUTPUT (String(" Height : ") + StringConverter::toString(base_image_height));
        DEBUG_OUTPUT (String(" Bpp : ") +  StringConverter::toString(base_image_bypp) );
        DEBUG_OUTPUT ( filename );
        ...
        Image resizer;
        uchar *SplittedData = 0;
        for (unsigned int i = 0; i <= height_split_count; i++)
        {
            for (unsigned int j = 0; j <= width_split_count; j++)
            {
                //SplittedData = new uchar[size_split_count * base_image_bypp];
				SplittedData = OGRE_ALLOC_T(uchar, size_split_count * base_image_bypp, MEMCATEGORY_GENERAL);
                // Blit copy from base image to sub-image
                BlitSubImage (BaseData,
                                SplittedData,
                                j * (page_size - 1),
