...
private :
    ...
    std::vector<Image> SplatMap; // source Splats Maps
    std::vector<Image> destAlphaMap; //destination Alpha Maps
    
    Image CoverageMap; // Synthesize the alpha map as one image if 4 color maps.
    
    std::vector<ColourValue> matColor;// average color value of splats map
    std::vector<Real> matHeight; // Height value upon which splat is chosen
